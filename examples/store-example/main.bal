...
service / on new http:Listener(9090) {
    ...
    resource isolated function get init () returns http:TemporaryRedirect {
    ...
    resource isolated function get callback (http:Caller caller, http:Request req) returns error? {
        ...
        if code is string {
            ...
            accessToken = check getRefreshToken(oauth2Config);
    ...
    resource isolated  function get test/token (http:Caller caller, http:Request req) returns error? {
...
function getRefreshToken(OAuth2Config oauth2Config) returns string|error {
    ...
    json|error refresh_token = response.refresh_token;

    if refresh_token is string {
